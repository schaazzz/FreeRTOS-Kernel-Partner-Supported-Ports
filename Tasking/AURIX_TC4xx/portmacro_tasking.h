...
#ifndef PORTMACRO_TC4X_TASKING_H
#define PORTMACRO_TC4X_TASKING_H
...
#define portSET_INTERRUPT_MASK_FROM_ISR()                        ulPortSetCCPN( configMAX_API_CALL_INTERRUPT_PRIORITY )
...
static void __attribute__( ( used, always_inline ) ) vPortSetICR( uint32_t ulICR )
{
    __disable();
    __mtcr( portCPU_ICR, ulICR );
    __enable();
}

static uint32_t __attribute__( ( used, always_inline ) ) ulPortSetCCPN( unsigned char ucCCPN )
{
    uint32_t ulICR;

    __disable();
    ulICR = __mfcr( portCPU_ICR );
    __mtcr( portCPU_ICR, ( ulICR & ~portCPU_ICR_CCPN_MSK ) | ( ucCCPN & portCPU_ICR_CCPN_MSK ) );
    __enable();
    return ulICR;
}
